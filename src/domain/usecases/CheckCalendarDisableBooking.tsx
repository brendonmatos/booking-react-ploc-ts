...
export class CheckCalendarDisableBooking {
	...
	execute(checkCalendarDisableBookingDto: CheckCalendarDisableBookingDto) {
		...
		if (startBookingDate) {
			const result = this.reservationsService.getBookingBeforeAndAfter(
				startBookingDate,
				checkCalendarDisableBookingDto.bookingId,
			);
			const isAfterAnyBooking = result.after
				? checkDate >= result.after.dateStart
				: false;
			const isBeforeAnyBooking = result.before
				? checkDate <= result.before.dateEnd
				: false;
			return isAfterAnyBooking || isBeforeAnyBooking;
		}

		const conflictingBookings = this.reservationsService.getBookingsConflicting(
			checkDate,
			checkDate,
			checkCalendarDisableBookingDto.bookingId,
		);
