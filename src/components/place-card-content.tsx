...
export function PlaceCardContent({
  ...
  return (
    <div className={cn("flex flex-col w-full ", {
      ...
      <div className={
          cn("relative bg-center bg-cover", 
            {
...
export const CardContentFlex = ({  place, orientation = "vertical" }: { orientation?: "horizontal" | "vertical", place: Place }) => {
  return (
    <>
      <h3 className={cn("", {
        "mt-10 scroll-m-20 pb-2 text-5xl font-semibold tracking-tight transition-colors first:mt-0": orientation === "horizontal",
      })}>{place.name}</h3>
      <span className={cn("font-semibold", {
        "text-2xl": orientation === "horizontal",
      })}>{formatCurrencyToHuman(place.price)}</span>
      <span className={cn("text-muted-foreground", {
        "text-lg": orientation === "horizontal",
      })}> per night without taxes</span>
    </>
  )
}